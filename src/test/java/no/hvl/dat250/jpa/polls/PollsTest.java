...
public class PollsTest {
    ...
    poll2.addVoteOption("Mamma mia: Nooooo!");  // 👈 ingen "unused variable"
    ...
    public void setUp() {
        EntityManagerFactory emf = new PersistenceConfiguration("polls")
                ...
                .property(PersistenceConfiguration.JDBC_URL,"jdbc:h2:file:./build/h2db/polls;AUTO_SERVER=TRUE")
    ...
    public void testUsers() {
        emf.runInTransaction(em -> {
            ...
            User maybeBob = em.createQuery(
                    "select u from User u where u.name like 'bob'", User.class)
    ...
    public void testOptions() {
        emf.runInTransaction(em -> {
            List<String> poll2Options = em.createQuery(
                    "select o.text from Poll p join p.options o join p.creator u " +
                            "where u.email = :mail order by o.presentationOrder", String.class)
                    .setParameter("mail", "eve@mail.org")
                    .getResultList();

            assertEquals(Arrays.asList("Yes! Yammy!", "Mamma mia: Nooooo!"), poll2Options);
        });
    }
}
